...
COYEAR_impVal <- eventMeta_totals_impValFull %>%
  ...
  mutate(coho_yearling_interp.linear = imputeTS::na_interpolation(ts(coho_yearling_obs_validation), option="linear"),
         coho_yearling_interp.stine = imputeTS::na_interpolation(ts(coho_yearling_obs_validation), option="stine"),
         coho_yearling_kal.structs = imputeTS::na_kalman(ts(coho_yearling_obs_validation), model="StructTS"),
         coho_yearling_kal.arima = imputeTS::na_kalman(ts(coho_yearling_obs_validation), model="auto.arima"),
         coho_yearling_MA.simp2 = imputeTS::na_ma(ts(coho_yearling_obs_validation), weighting="simple", k=2),
         coho_yearling_MA.simp3 = imputeTS::na_ma(ts(coho_yearling_obs_validation), weighting="simple", k=3),
         coho_yearling_MA.linear2 = imputeTS::na_ma(ts(coho_yearling_obs_validation), weighting="linear", k=2),
         coho_yearling_MA.linear3 = imputeTS::na_ma(ts(coho_yearling_obs_validation), weighting="linear", k=3),
         coho_yearling_MA.exp2 = imputeTS::na_ma(ts(coho_yearling_obs_validation), weighting="exponential", k=2),
         coho_yearling_MA.exp3 = imputeTS::na_ma(ts(coho_yearling_obs_validation), weighting="exponential", k=3),
...
# Save as PDF: 
pdf(file = here::here("outputs", "figures", "RST infill-CPUE-abundance", "Imputation diagnostic plot - Coho yearling.pdf"),   # The directory you want to save the file in
...
print(
  ggplot() +
    ...
    labs(x="", y="Yearling Coho catch", colour="Imputation method", fill="Imputation method", size="Data type", shape="Data type") +
    theme_bw() +
