...
CNHO_impVal <- eventMeta_totals_impValFull %>%
  select(-c(chinook_natural_obs:coho_alevin_obs, chinook_natural_obs_validation:chum_fry_obs_validation)) %>%
  filter(year != 2023) %>%
  group_by(year) %>%
  mutate(chinook_hatchery_interp.linear = imputeTS::na_interpolation(ts(chinook_hatchery_obs_validation), option="linear"),
         chinook_hatchery_interp.stine = imputeTS::na_interpolation(ts(chinook_hatchery_obs_validation), option="stine"),
         chinook_hatchery_kal.structs = imputeTS::na_kalman(ts(chinook_hatchery_obs_validation), model="StructTS"),
         chinook_hatchery_kal.arima = imputeTS::na_kalman(ts(chinook_hatchery_obs_validation), model="auto.arima"),
         chinook_hatchery_MA.simp2 = imputeTS::na_ma(ts(chinook_hatchery_obs_validation), weighting="simple", k=2),
         chinook_hatchery_MA.simp3 = imputeTS::na_ma(ts(chinook_hatchery_obs_validation), weighting="simple", k=3),
         chinook_hatchery_MA.linear2 = imputeTS::na_ma(ts(chinook_hatchery_obs_validation), weighting="linear", k=2),
         chinook_hatchery_MA.linear3 = imputeTS::na_ma(ts(chinook_hatchery_obs_validation), weighting="linear", k=3),
         chinook_hatchery_MA.exp2 = imputeTS::na_ma(ts(chinook_hatchery_obs_validation), weighting="exponential", k=2),
         chinook_hatchery_MA.exp3 = imputeTS::na_ma(ts(chinook_hatchery_obs_validation), weighting="exponential", k=3),
         # infill_type = case_when(is.na(chinook_hatchery_obs_validation) & !is.na(chinook_hatchery_obs) ~ "ground truth",
         #                         TRUE ~ "known value")
  )  %>%
  pivot_longer(cols=c(chinook_hatchery_obs:chinook_hatchery_MA.exp3), names_to = "data_series", values_to = "value") 



# ----- VISUALIZE ------------

plot_cnHO_imputation_validation <- 
  ggplot() +
  geom_point(data=CNHO_impVal %>% 
               filter(!is.na(value), estimate_type=="observed" & data_series=="chinook_hatchery_obs"),
  ...
  geom_jitter(data=CNHO_impVal %>% 
                filter(!is.na(value), validation_type=="validation" & data_series!="chinook_hatchery_obs"),
  ...
  labs(x="", y="Hatchery Chinook count", colour="Imputation method", fill="Imputation method", size="Data type", shape="Data type") +
  theme_bw() +
...
# Save as PDF: 
pdf(file = here::here("outputs", "figures", "Imputation diagnostic plot - Hatchery Chinook.pdf"),   # The directory you want to save the file in
...
infill_evaluation_table.CNHO <- CNHO_impVal %>% 
  pivot_wider(names_from = data_series, values_from = value) %>%
  filter(validation_type=="validation") %>% 
  select(-c(chinook_hatchery_obs_validation)) %>%
  pivot_longer(cols=c(chinook_hatchery_interp.linear:chinook_hatchery_MA.exp3), names_to = "infill_method", values_to = "infill_value") %>%
  mutate(# Just doing this to show my work for future me: 
    Error = (chinook_hatchery_obs - infill_value),   # Calculate error
    `Error/Obs` = Error/chinook_hatchery_obs,        # Calculate error divided by observed value (part of MAPE)
  ...
  mutate(#        n_w0 = n(),                       # This is all long-hand work to ensure I understood how the metric calculations work. For simplicity of coding, I installed the Metrics package and verified my work 
    ...
    MAE_w0 = Metrics::mae(chinook_hatchery_obs, infill_value),                               # Calculate MAE including the observed zero to see if it has an affect
    MAE_no0 = Metrics::mae(chinook_hatchery_obs[chinook_hatchery_obs>0], infill_value),      # Calculate MAE excluding the observed zero to see if it has an affect
    MASE = Metrics::mase(chinook_hatchery_obs, infill_value, step_size = 1)                  # Calculate MASE. Step of 1 indicates the previous day is informative for the naive model. For example, a step of 4 would be used for quarterly work where you imply the last quarter was more informative.
  ) %>% 
  print()
...
infill_summary.CNHO <- full_join(infill_evaluation_table.CNHO %>% 
                                 filter(chinook_hatchery_obs>0) %>%
                                 group_by(year, infill_method) %>%
                                 mutate(MAPE = Metrics::mape(chinook_hatchery_obs, infill_value)) %>% 
                                 group_by(year, infill_method) %>%
                                 summarize(MAPE=unique(MAPE)),
                               infill_evaluation_table.CNHO %>%
                                 group_by(year, infill_method) %>%
