...
CM_impVal <- eventMeta_totals_impValFull %>%
  select(-c(chinook_natural_obs, coho_subyearling_obs:coho_yearling_obs_validation)) %>%
  filter(year != 2023) %>%
  group_by(year) %>%
  mutate(chum_fry_interp.linear = imputeTS::na_interpolation(ts(chum_fry_obs_validation), option="linear"),
         chum_fry_interp.stine = imputeTS::na_interpolation(ts(chum_fry_obs_validation), option="stine"),
         chum_fry_kal.structs = imputeTS::na_kalman(ts(chum_fry_obs_validation), model="StructTS"),
         chum_fry_kal.arima = imputeTS::na_kalman(ts(chum_fry_obs_validation), model="auto.arima"),
         chum_fry_MA.simp2 = imputeTS::na_ma(ts(chum_fry_obs_validation), weighting="simple", k=2),
         chum_fry_MA.simp3 = imputeTS::na_ma(ts(chum_fry_obs_validation), weighting="simple", k=3),
         chum_fry_MA.linear2 = imputeTS::na_ma(ts(chum_fry_obs_validation), weighting="linear", k=2),
         chum_fry_MA.linear3 = imputeTS::na_ma(ts(chum_fry_obs_validation), weighting="linear", k=3),
         chum_fry_MA.exp2 = imputeTS::na_ma(ts(chum_fry_obs_validation), weighting="exponential", k=2),
         chum_fry_MA.exp3 = imputeTS::na_ma(ts(chum_fry_obs_validation), weighting="exponential", k=3),
         # infill_type = case_when(is.na(chum_fry_obs_validation) & !is.na(chum_fry_obs) ~ "ground truth",
         #                         TRUE ~ "known value")
  )  %>%
  pivot_longer(cols=c(chum_fry_obs:chum_fry_MA.exp3), names_to = "data_series", values_to = "value") 



# ----- VISUALIZE ------------

plot_chum_imputation_validation <- 
  ggplot() +
  geom_point(data=CM_impVal %>% 
               filter(!is.na(value), estimate_type=="observed" & data_series=="chum_fry_obs"),
  ...
  geom_jitter(data=CM_impVal %>% 
                filter(!is.na(value), validation_type=="validation" & data_series!="chum_fry_obs"),
  ...
  labs(x="", y="Chum count", colour="Imputation method", fill="Imputation method", size="Data type", shape="Data type") +
  theme_bw() +
...
# Save as PDF: 
pdf(file = here::here("outputs", "figures", "Imputation diagnostic plot - Chum.pdf"),   # The directory you want to save the file in
...
infill_evaluation_table.CM <- CM_impVal %>% 
  pivot_wider(names_from = data_series, values_from = value) %>%
  filter(validation_type=="validation") %>% 
  select(-c(chum_fry_obs_validation)) %>%
  pivot_longer(cols=c(chum_fry_interp.linear:chum_fry_MA.exp3), names_to = "infill_method", values_to = "infill_value") %>%
  mutate(# Just doing this to show my work for future me: 
    Error = (chum_fry_obs - infill_value),   # Calculate error
    `Error/Obs` = Error/chum_fry_obs,        # Calculate error divided by observed value (part of MAPE)
  ...
  mutate(#        n_w0 = n(),                       # This is all long-hand work to ensure I understood how the metric calculations work. For simplicity of coding, I installed the Metrics package and verified my work 
    ...
    MAE_w0 = Metrics::mae(chum_fry_obs, infill_value),                               # Calculate MAE including the observed zero to see if it has an affect
    MAE_no0 = Metrics::mae(chum_fry_obs[chum_fry_obs>0], infill_value),      # Calculate MAE excluding the observed zero to see if it has an affect
    MASE = Metrics::mase(chum_fry_obs, infill_value, step_size = 1)                  # Calculate MASE. Step of 1 indicates the previous day is informative for the naive model. For example, a step of 4 would be used for quarterly work where you imply the last quarter was more informative.
  ) %>% 
  print()
...
infill_summary.CM <- full_join(infill_evaluation_table.CM %>% 
                                     filter(chum_fry_obs>0) %>%
                                     group_by(year, infill_method) %>%
                                     mutate(MAPE = Metrics::mape(chum_fry_obs, infill_value)) %>% 
                                     group_by(year, infill_method) %>%
                                     summarize(MAPE=unique(MAPE)),
                                   infill_evaluation_table.CM %>%
                                     group_by(year, infill_method) %>%
...
write.csv(infill_summary.CM, file=here::here("outputs", "R_OUT - imputation method metrics Chum.csv"), row.names=F)
